...
Because empirical academic projects have a lot in common with software projects inasmuch that there is a certain expectation and quality control, but the outcome is not known in advance. Essentially in agile project management you roughly define an outcome along the lines of a minimum viable product (MVP), so you do not end up with nothing after a year of back and forth. 
Then you'd meet regularly, let's say every 10 days, to talk about what you worked on within the last 10 days and what your plans are for the next 10 days. Decisions on what to work on are made, issues are raised and solution to problems will be sketched out and discussed bilaterally or in small groups. That way the team stays flexible. In professional setups, often different roles, e.g., stakeholder or moderator (in regiligous, certified level SCRUM: the SCRUM master) are assigned. 

Major git platforms ship with a decent, carpentry level project management project management GUI. The issue tracker is at the core of this. If you use it the minimal way, it's simply a colorful to-do list. 
